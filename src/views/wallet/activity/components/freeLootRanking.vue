    ...
    <div class="goldTab">
      ...
      <div class="table">
        <div class="goldTableSelect">
          <n-select @update:value="selectRoundid"  v-model:value="freeLootRankingData.roundid" :options="freeLootRankingData.roundidOptions" />
        </div>
        <n-data-table striped :bordered="false" virtual-scroll :max-height="500" :columns="freeLootRankingData.tableHeader"
        ...
        <n-button v-if="freeLootRankingData.list.length >= 50" @click="pushTableData" tertiary class="planButton">
          点击加载更多
        </n-button>
      </div>
    </div>
  ...
  tableHeader: [
    {
      ...
      render(row: any) {
        ...
        switch (row.rank) {
          case '1' :
            hRender = h(Imgt, { src: '/img/wallet/gameTop1.png', class: `rankingImg` });
            break;
          case '2' :
            hRender = h(Imgt, { src: '/img/wallet/gameTop2.png', class: `rankingImg` });
            break;
          case '3' :
            hRender = h(Imgt, { src: '/img/wallet/gameTop3.png', class: `rankingImg` });
            break;
          default:
    ...
    {
      title: '头像', key: 'head_icon',
      render(row: any) {
        // /img/head_icons/${roleInfo.head_photo}.webp
        return h(Imgt, { src:`/img/head_icons/${row.head_icon}.webp`, class: `avatarImg avatarImg-${row.avatar}` });
      },
    },
    { title: '昵称', key: 'username' },
    {
      title: '试玩金币', key: 'score',
      render(row: any) {
        return h('span', { class: 'yellowText', innerHTML: `试玩金币：<i>${row.score}</i>` });
      },
    },
    {
      title: '本期奖励', key: 'reward',
      render(row: any) {
        return h('span', { class: 'yellowText', innerHTML: `奖励：<i>${row.reward}</i>` });
      },
    },
  ],
});
...
onUnmounted(() => {
  MessageEvent2.removeMsgEvent(NetMsgType.msgType.msg_notify_free_treasure_his, null);
});
      ...
      .goldTableSelect {
        ...
        .n-select {
          ...
          :deep(.n-base-selection) {
            ...
            .n-base-selection__border, .n-base-selection__state-border, .n-base-selection-label {
