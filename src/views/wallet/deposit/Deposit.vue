<template>
  <CommonForm :title="t('deposit_page_deposit')" :submit-text="t('deposit_page_arrival')"
              :form-ref="'depositFormRef'"
              :tab-list="tabList" :active-tab="activeTab" :form="form" :show-form="showDeposit"
              :showTab="false"
              @change-tab="changeTab" @on-close="onClose" @on-submit="onSubmit"
...
const handleShopInfoRes = (rs: TShopInfo) => {
  ...
  rs.discount_list.forEach((dc: TDiscount) => dcList.value.push({
    label: dc.name,
